  ...
  methods: {
    proceed: function (value) {
      if (!window.WEB3.utils.isAddress(this.$data.address)) {
        ...
        this.$router.push('/phase1new-account/' + window.WEB3.utils.toChecksumAddress(this.$data.address) + '/10')
        this.$root.models.remoteweb3 = true
      }
    },
    accountSelected: function (value) {
      this.$router.push('/phase1new-account/' + value + '/10')
    },
